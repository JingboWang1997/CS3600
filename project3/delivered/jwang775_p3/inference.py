...
class ExactInference(InferenceModule):
    ...
    def observe(self, observation, gameState):
        ...
        #   = probability of noisyDistance being this value given the GHOST being at each possible position
        # P(TrueDistance | noisyDistance)
        #   = probability of GHOST being at each position given the noisyDistance
        # P(TrueDistance)
        #   = probability of GHOST being at each position without any new information
        #
        # self.beliefs store P(TrueDistance)
        # allPossible stores P(TrueDistance | noisyDistance)
        # emissionModel[TrueDistance] = P(noisyDistance | TrueDistance)
        #
        # P(TrueDistance | noisyDistance) = P(noisyDistance | TrueDistance) P(TrueDistance) * a
        # a = normalize
    ...
    def elapseTime(self, gameState):
        ...
        *** GORY DETAIL AHEAD ***
        ...
        # belief[p] -> P(ghost at p @ time t)
        # newPosDist -> P(ghost at p @ time t + 1 | ghost at oldPos @ time t )
        # want: P(ghost at p @ time t + 1)

        # P(ghost at p @ time t + 1) = SUM(all possible oldPos){P(ghost at p @ time t + 1 | ghost at oldPos @ time t = True) * P(ghost at oldPos @ time t)}
        # allPossible = SUM(all possible oldPos){newPosDist[p] * belief[oldPos]
        ...
        for p in self.legalPositions:
            ...
            for newPos, prob in newPosDist.items():
                realProb = self.beliefs[p] * prob
                allPossible[newPos] += realProb
        allPossible.normalize()
        self.beliefs = allPossible
...
class ParticleFilter(InferenceModule):
    ...
    def initializeUniformly(self, gameState):
        ...
        while (len(self.particles) < self.numParticles):
            self.particles.append(posList[counter])
            counter += 1
            if (counter == len(posList)):
                counter = 0
    ...
    def observe(self, observation, gameState):
        ...
        # emissionModel[TrueDistance] = P(noisyDistance | TrueDistance)
        # P(noisy distance is true given the evidence of at the
        ...
        allPossible = util.Counter()
        #previous = self.getBeliefDistribution()
    ...
    def elapseTime(self, gameState):
        ...
        for pos in self.particles:
            ...
            result.append(util.sample(newPosDist))
...
class JointParticleFilter:
    ...
    def initializeParticles(self):
        ...
        counter = 0
        while (len(self.particles) < self.numParticles):
            self.particles.append(allPos[counter])
            counter += 1
            if (counter == len(allPos)):
                counter = 0
    ...
    def observeState(self, gameState):
        ...
        ** Remember ** We store particles as tuples, but to edit a specific particle,
        ...
        # each key is a tuple of positions

        for singlePosSet in self.particles:
            singlePos = 1
            counter = 0
            for hearing in noisyDistances:
                if hearing == None: # take out the counter one in each tuple
                    singlePosSet = self.getParticleWithGhostInJail(singlePosSet, counter)
                    #singlePos += 1
                else:
                    TrueDistance = util.manhattanDistance(singlePosSet[counter], pacmanPosition)
                    singlePos *= emissionModels[counter][TrueDistance]
                counter += 1
            allPossible[singlePosSet] += singlePos
        ...
        **Note** that you may need to replace "prevGhostPositions" with the
        ...
        for oldParticle in self.particles:
            ...
            for i in range(self.numGhosts):
                ...
                newPosSet.append(util.sample(newPosDist))
            newParticle = newPosSet
