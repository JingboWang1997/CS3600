...
class NeuralNet(object):                                    
    ...
    def feedForward(self, inActs):
        ...
        for hiddenLayer in self.hiddenLayers:
            newLayer = []
            for p in hiddenLayer:
                newLayer.append(p.sigmoidActivation(curLayer))
            curLayer = newLayer
            result.append(curLayer)

        outLayer = []
        for pFinal in self.outputLayer:
            outLayer.append(pFinal.sigmoidActivation(curLayer))

        return [inActs] + result + [outLayer]
        ...
        for example in examples:#for each example
            ...
            for layerNum in xrange(self.numHiddenLayers-1,-1,-1):
                ...
                for neuronNum in xrange(len(layer)):
                    ...
                    sum = 0
                    for nextNum in xrange(len(nextLayer)):
                        sum += (nextLayer[nextNum].weights[neuronNum+1] * deltas[0][nextNum])
                    delta = gPrime * sum
...
def buildNeuralNet(examples, alpha=0.1, weightChangeThreshold = 0.00008,hiddenLayerList = [1], maxItr = sys.maxint, startNNet = None):
    ...
    for test in examplesTest:
        result = nnet.feedForward(test[0])
        actual = result[-1]
        actual = [round(x) for x in actual]
        if actual == test[1]:
            testCorrect += 1
        else:
            testError += 1
